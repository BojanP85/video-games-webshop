...
<html>
  ...
  <body>
    <?php
      ...
      $izdvojeno = mysqli_query($link, "SELECT * FROM proizvodi WHERE izdvojeno = 1");
      ...
      <div class="container">
        ...
        <ul class="nav navbar-nav">
          <?php while ($tk_red = mysqli_fetch_assoc($tk_rezultat)) { ?>
            <?php
              $tk_id = $tk_red["id"]; /* Pri svakom prolasku kroz WHILE petlju (32. red), u promenljivu $tk_id smeštamo id vrednost trenutne top-kategorije
                                         (za "Playstation" id ima vrednost 1, za "XBOX" id ima vrednost 2 itd.)
                                         Ovo nam je bitno kako bismo u novoj WHILE petlji (42. red) omogućili pristup pod-kategorijama. */
    ...
    <div class="container-fluid">
      ...
      <div class="col-md-8 text-center glavni_okvir">
        <div class="row"><br>
          <h2 class="text-center" style="margin-top: -35px">Izdvajamo iz ponude</h2><br><br>
          <?php while ($izdvojeno_red = mysqli_fetch_assoc($izdvojeno) ) { ?>
           <div class="col-md-4">
            <h4 style="margin-top: 35px"><?php echo $izdvojeno_red["naziv"]; ?></h4>
            <img src="slike/<?php echo $izdvojeno_red["slika"]; ?>" alt="<?php echo $izdvojeno_red["naziv"]; ?>"/>
            <p class="stara-cena text-danger">Stara cena: <s><?php echo $izdvojeno_red["stara_cena"]; ?>din</s></p>
            <p class="nova-cena">Cena: <?php echo $izdvojeno_red["cena"]; ?>din</p>
            <button type="button" class="btn btn-sm btn-success" onclick="detalji(<?php echo $izdvojeno_red["id"]; ?>)">Detalji</button>
           </div>
          <?php } ?>
        </div>
      </div>

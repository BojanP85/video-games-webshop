...
<html>
  ...
  <body>
    <?php
      ...
      $kategorija_id = (($_POST["kat"] != "") ? $_POST["kat"] : "");
      if($kategorija_id == "") {
        $upit = "SELECT * FROM proizvodi";
      } else {
        $upit .= " WHERE proizvodi.kategorije = '{$kategorija_id}'";
      }

      $cena_filter = (($_POST["cena_filter"] != "") ? $_POST["cena_filter"] : "");
      $min_cena  = (($_POST["min_cena"] != "") ? $_POST["min_cena"] : "");
      $max_cena = (($_POST["max_cena"] != "") ? $_POST["max_cena"] : "");
      $proizvodi_akcija = (($_POST["proizvodi_akcija"] != "") ? $_POST["proizvodi_akcija"] : "");
      $zanr_filter = (($_POST["zanr_filter"] != "") ? $_POST["zanr_filter"] : "");

      if($proizvodi_akcija == "akcija") {
        $upit .= " AND proizvodi.stara_cena <> 'NULL'";
      }
      if($min_cena != "") {
        $upit .= " AND proizvodi.cena >= '{$min_cena}'";
      }
      if($max_cena != "") {
        $upit .= " AND proizvodi.cena <= '{$max_cena}'";
      }
      if($zanr_filter != "") {
        $upit .= " AND proizvodi.zanr = '{$zanr_filter}'";
      }
      if($cena_filter == "low") {
        $upit .= " ORDER BY proizvodi.cena ASC";
      }
      if($cena_filter == "high") {
        $upit .= " ORDER BY proizvodi.cena DESC";
      }

      $proizvodi_rezultat = mysqli_query($link, $upit);
